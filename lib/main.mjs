...
// PDFViewer._resetView empties this._pages and drops the page buffer without
// calling destroy() on the views it is discarding, so every rendered canvas
// stays alive until the garbage collector gets round to it. That is harmless
// in a viewer you open once, and it is not what this extension does: a LaTeX
// or Typst loop reloads the document on every rebuild, and each reload leaves
// another set of backing stores behind. Freeing them at the point we already
// know they are dead turns that sawtooth into a flat line.
class TidyPDFViewer extends PDFViewer {
  _resetView() {
    for (const pageView of this._pages ?? []) {
      try {
        pageView.destroy();
      } catch (error) {
        // A page already torn down is not an error worth failing a reload for.
        console.warn('PDF Preview: failed to destroy page view', error);
      }
    }
    super._resetView();
  }
...
// Order, labels and page colours live in pageMode.mjs. They are imported
// rather than restated: keeping a second copy here is what kept breaking.
const THEME_VALUES = PAGE_MODE_VALUES;
...
function applyPageColorVariables(viewer, theme) {
  const pageColors = pageColorsForPageMode(theme);
...
class PdfPreviewApp {
  ...
  setupEvents() {
    ...
    // Right-click is the same escape hatch without a modifier to know about.
    // The cycle deliberately skips plain pages, so getting back to them has to
    // be reachable by someone who never reads a tooltip.
    this.elements.themeToggle.addEventListener('contextmenu', (event) => {
      event.preventDefault();
      this.cyclePageTheme({ clear: true });
    });
    ...
    window.addEventListener('message', (event) => {
      ...
      if (
      ...
      } else if (message.type === 'clear-page-mode') {
        this.cyclePageTheme({ clear: true });
        ...
        message.type === 'appearance-state' &&
        typeof message.theme === 'string'
      ) {
        this.setPageMode(message.theme);
      } else if (
  ...
  cyclePageTheme({ clear = false } = {}) {
    this.flushPersistViewState();
    const theme = clear
      ? clearedPageMode(this.appearance.theme)
      : nextPageMode(this.appearance.theme);
    this.setPageMode(theme, { persist: true });
  }

  /// The single way the viewer changes page mode.
  ///
  /// The wheel, the clear shortcut and the host's re-seed after a webview
  /// reload all land here, so none of them can drift from the others. Only a
  /// user action persists: echoing a re-seed straight back to the setting
  /// would let a stale webview overwrite the live value.
  setPageMode(theme, { persist = false } = {}) {
    ...
    if (
      !this.pdfDocument ||
      !this.applyPageColors(pageColorsForPageMode(theme))
    ) {
      this.loadDocument({ restoreView: true, retryOnFailure: true });
    }
  }

  /// Repaint the open pages in the new page colors.
  ///
  /// Page colors are baked into the rendered canvas, not applied over it, so a
  /// colour change is only visible once the canvas is drawn again. Setting the
  /// colours and calling PDFViewer.refresh() is not enough: refresh routes
  /// through PDFPageView.update(), which — when nothing about the geometry has
  /// changed — resets with keepCanvasWrapper and therefore never calls
  /// _resetCanvas(). The old bitmap survives. That is why clearing Sepia used
  /// to turn the page background white and leave the text brown.
  ///
  /// Dropping each canvas explicitly is what makes the change appear.
  applyPageColors(pageColors) {
    ...
    for (const pageView of pages) {
      ...
      try {
        // No keepCanvasWrapper: the canvas must go, or the old colours stay.
        pageView.reset();
      } catch (error) {
        console.warn('PDF Preview: failed to reset page view', error);
        return false;
      }
    }
  ...
  updateThemeToggle() {
    const index = pageModeIndex(this.appearance.theme);
    const currentLabel = pageModeLabel(this.appearance.theme);
    const nextLabel = pageModeLabel(nextPageMode(this.appearance.theme));
    const enabled = this.appearance.theme !== CLEAR_PAGE_MODE;
    // Naming the whole wheel and where you are on it is the point: with four
    // stops and a one-word label, "did that press do anything?" was guesswork.
    const description =
      `Page mode ${index + 1} of ${PAGE_MODE_COUNT}: ${currentLabel} — ` +
      `click for ${nextLabel} (${PAGE_MODES.map((mode) => mode.label).join(' → ')} → …), ` +
      `Shift+click or right-click for plain pages`;
    ...
    this.updateThemeWheel(index);
  }

  /// One dot per stop, the current one filled: the wheel you are on, visible
  /// without hovering for a tooltip.
  updateThemeWheel(index) {
    const wheel = this.elements.themeToggle.querySelector('.wheel');
    if (!wheel) {
      return;
    }
    if (wheel.childElementCount !== PAGE_MODE_COUNT) {
    ...
    for (const [position, dot] of [...wheel.children].entries()) {
